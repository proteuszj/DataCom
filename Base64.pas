...
const
  base64_encoding : PChar = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/';
  ...
  {将二进制流转换为basee64编码}
  function Base64Enc(var buf : TMemoryStream;var text : String; size : Integer) : Integer;
  {将basee64编码转换为二进制流}
  function Base64Dec(OutputBuffer : Pointer;var text : String; size : Integer) : Integer;
  {/*base64解码的实现*/}
  function GetBase64Value(ch : Char) : Cardinal;
...
/*
 *输入参数：
 *var buf : TMemoryStream	 待编码的数据流
 *var text : String             base64编码后的数据流
 *int  size		数据流的长度
 *返回值：
 *int bufsize		编码后的数据流长度
 */}
function Base64Enc(var buf : TMemoryStream;var text : String; size : Integer) : Integer;
var
//  InPtr : ^Byte;
...
//  InPtr := @buf;
  ...
  while (size >0) do begin
    buf.Position := (pos) * 3;
    buf.Read(s1,1);
    text := text + base64_encoding[(s1 shr 2) and $3F];
    if(size>2) then begin
      buf.Position := pos * 3 + 1;
      buf.Read(s2,1);
      buf.Position := pos * 3 + 2;
      buf.Read(s3,1);
      text := text + base64_encoding[((s1 and $3) shl 4) or (s2 shr 4)];
      text := text + base64_encoding[((s2 and $F) shl 2) or (s3 shr 6)];
      text := text + base64_encoding[s3 and $3F];
    end else begin
      if size = 1 then begin
        text := text + base64_encoding[(s1 and $3) shl 4];
        text := text + '=';
        text := text + '=';
...
/*
 *输入参数：
 *OutputBuffer : Pointer	 解码后的数据流
 *var text : String              base64编码的待解码数据流
 *int  size		数据流的长度
 *返回值：
 *int bufsize		编码后的数据流长度
 */}
...
begin
  ...
  inStream.SetSize(Length(text));
  try
    CopyMemory(inStream.Memory, PChar(text), Length(text));
    MimeDecodeStream(inStream, outStream);
    //outStream.SaveToFile('2.jpg');
    CopyMemory(OutputBuffer,outStream.Memory, outStream.Size);
